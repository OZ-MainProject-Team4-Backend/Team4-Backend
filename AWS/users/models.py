...
class EmailVerification(models.Model):
    ...
    code = models.CharField(max_length=100) # 토큰 길이가 10자를 넘을 수 있으므로 100으로 변경
...
class Token(models.Model):
    id = models.AutoField(primary_key=True)
    user = models.ForeignKey(User, on_delete=models.CASCADE)
    access_jwt = models.CharField(max_length=500, blank=True, null=True) # JWT 길이가 200을 넘을 수 있으므로 500으로 변경
    refresh_jwt = models.CharField(max_length=500, blank=True, null=True) # JWT 길이가 200을 넘을 수 있으므로 500으로 변경
